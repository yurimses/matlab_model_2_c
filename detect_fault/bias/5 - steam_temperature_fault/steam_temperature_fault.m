...
% LER E SALVAR
steam_temperature_bias = readtable('steam_temperature_bias5_4.csv');

%% DADOS

total_valores = sprintf("Total de valores = %d", height(steam_temperature_bias));
disp(total_valores);

%% DIVISÃO PARA TREINAMENTO E TESTE (SEQUENCIAL 70/30)

% Percentagem de treino
perc_treino = round(0.7*(height(steam_temperature_bias)));

% Atribuindo features (x) e saídas (y)
x = steam_temperature_bias(: , 2);
y = steam_temperature_bias(: , 3);
...
% cv = cvpartition(size(steam_temperature_bias,1),'Holdout',0.3); 
% 
% % Extrai índices do teste 
% idx = cv.test;
% 
% % Usa índices para partição
% dadosTreino = steam_temperature_bias(~idx,:);
% dadosTeste = steam_temperature_bias(idx,:);
% 
