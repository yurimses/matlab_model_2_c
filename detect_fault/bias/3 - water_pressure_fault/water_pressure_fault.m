...
% LER E SALVAR
water_pressure_bias = readtable('water_pressure_bias5_4.csv');

%% DADOS

total_valores = sprintf("Total de valores = %d", height(water_pressure_bias));
disp(total_valores);

%% DIVISÃO PARA TREINAMENTO E TESTE (SEQUENCIAL 70/30)

% Percentagem de treino
perc_treino = round(0.7*(height(water_pressure_bias)));

% Atribuindo features (x) e saídas (y)
x = water_pressure_bias(: , 2);
y = water_pressure_bias(: , 3);
...
% cv = cvpartition(size(water_pressure_bias,1),'Holdout',0.3); 
% 
% % Extrai índices do teste 
% idx = cv.test;
% 
% % Usa índices para partição
% dadosTreino = water_pressure_bias(~idx,:);
% dadosTeste = water_pressure_bias(idx,:);
% 
