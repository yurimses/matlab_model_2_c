...
% LER E SALVAR
steam_flow_bias = readtable('steam_flow_bias5_27.csv');

%% DADOS

total_valores = sprintf("Total de valores = %d", height(steam_flow_bias));
disp(total_valores);

%% DIVISÃO PARA TREINAMENTO E TESTE (SEQUENCIAL 70/30)

% Percentagem de treino
perc_treino = round(0.7*(height(steam_flow_bias)));
...
% cv = cvpartition(size(steam_flow_bias,1),'Holdout',0.3); 
% 
% % Extrai índices do teste 
% idx = cv.test;
% 
% % Usa índices para partição
% dadosTreino = steam_flow_bias(~idx,:);
% dadosTeste = steam_flow_bias(idx,:);
% 
