...
% LER E SALVAR
water_flow_bias = readtable('water_flow_bias5_27.csv');

%% DADOS

total_valores = sprintf("Total de valores = %d", height(water_flow_bias));
disp(total_valores);

%% DIVISÃO PARA TREINAMENTO E TESTE (SEQUENCIAL 70/30)

% Percentagem de treino
perc_treino = round(0.7*(height(water_flow_bias)));
...
% cv = cvpartition(size(water_flow_bias,1),'Holdout',0.3); 
% 
% % Extrai índices do teste 
% idx = cv.test;
% 
% % Usa índices para partição
% dadosTreino = water_flow_bias(~idx,:);
% dadosTeste = water_flow_bias(idx,:);
% 
