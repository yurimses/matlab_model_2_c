...
% LER E SALVAR
water_temperature_bias = readtable('water_temperature_bias5_1.csv');

%% DADOS

total_valores = sprintf("Total de valores = %d", height(water_temperature_bias));
disp(total_valores);

%% DIVISÃO PARA TREINAMENTO E TESTE (SEQUENCIAL 70/30)

% Percentagem de treino
perc_treino = round(0.7*(height(water_temperature_bias)));

% Atribuindo features (x) e saídas (y)
x = water_temperature_bias(: , 2);
y = water_temperature_bias(: , 3);
...
% cv = cvpartition(size(water_temperature_bias,1),'Holdout',0.3); 
% 
% % Extrai índices do teste 
% idx = cv.test;
% 
% % Usa índices para partição
% dadosTreino = water_temperature_bias(~idx,:);
% dadosTeste = water_temperature_bias(idx,:);
% 
...
% Acurácia
var1 = sum(resultado == y_teste.bias);
var2 = height(y_teste)*100;
acuracia = sum(resultado == y_teste.bias)/height(y_teste)*100;
