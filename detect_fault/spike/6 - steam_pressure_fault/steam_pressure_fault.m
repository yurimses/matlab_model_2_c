...
% LER E SALVAR
steam_pressure_spike = readtable('dynamicwin_steampressure.csv');
%save('water_flow_spike.mat')
%load water_flow_spike

%% DADOS

total_valores = sprintf("Total de valores = %d", height(steam_pressure_spike));
total_falhas = sprintf("Total de falhas = %d", height(steam_pressure_spike)-44643);
disp(total_valores);
%disp(total_falhas);
...
% perc_treino = round(0.7*(height(steam_pressure_spike)));
% 
% % Atribuindo features (x) e saídas (y)
% x = steam_pressure_spike(: , 2);
% y = steam_pressure_spike(: , 3);
% 
% % Treino (70%)
...
cv = cvpartition(size(steam_pressure_spike,1),'Holdout',0.3); 

% Extrai índices do teste 
idx = cv.test;

% Usa índices para partição
dadosTreino = steam_pressure_spike(~idx,:);
dadosTeste = steam_pressure_spike(idx,:);
