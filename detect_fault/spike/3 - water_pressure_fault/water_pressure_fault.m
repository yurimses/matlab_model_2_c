...
% LER E SALVAR
water_pressure_spike = readtable('dynamicwin_waterpressure.csv');
%save('water_flow_spike.mat')
%load water_flow_spike

%% DADOS

total_valores = sprintf("Total de valores = %d", height(water_pressure_spike));
total_falhas = sprintf("Total de falhas = %d", height(water_pressure_spike)-44643);
disp(total_valores);
%disp(total_falhas);
...
% perc_treino = round(0.7*(height(water_pressure_spike)));
% 
% % Atribuindo features (x) e saídas (y)
% x = water_pressure_spike(: , 2);
% y = water_pressure_spike(: , 3);
% 
% % Treino (70%)
...
cv = cvpartition(size(water_pressure_spike,1),'Holdout',0.3); 

% Extrai índices do teste 
idx = cv.test;

% Usa índices para partição
dadosTreino = water_pressure_spike(~idx,:);
dadosTeste = water_pressure_spike(idx,:);
...
%modelo = fitctree(x_treino, y_treino);
