...
% LER E SALVAR
water_temperature_drift = readtable('water_temperature_drift5_1.csv');

%% DADOS

total_valores = sprintf("Total de valores = %d", height(water_temperature_drift));
disp(total_valores);

%% DIVISÃO PARA TREINAMENTO E TESTE (SEQUENCIAL 70/30)

% Percentagem de treino
perc_treino = round(0.7*(height(water_temperature_drift)));

% Atribuindo features (x) e saídas (y)
x = water_temperature_drift(: , 2);
y = water_temperature_drift(: , 3);
...
% cv = cvpartition(size(water_temperature_drift,1),'Holdout',0.3); 
% 
% % Extrai índices do teste 
% idx = cv.test;
% 
% % Usa índices para partição
% dadosTreino = water_temperature_drift(~idx,:);
% dadosTeste = water_temperature_drift(idx,:);
% 
