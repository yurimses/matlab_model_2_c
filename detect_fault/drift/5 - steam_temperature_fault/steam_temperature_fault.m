...
% LER E SALVAR
steam_temperature_drift = readtable('steam_temperature_drift5_4.csv');

%% DADOS

total_valores = sprintf("Total de valores = %d", height(steam_temperature_drift));
disp(total_valores);

%% DIVISÃO PARA TREINAMENTO E TESTE (SEQUENCIAL 70/30)

% Percentagem de treino
perc_treino = round(0.7*(height(steam_temperature_drift)));

% Atribuindo features (x) e saídas (y)
x = steam_temperature_drift(: , 2);
y = steam_temperature_drift(: , 3);
...
% cv = cvpartition(size(steam_temperature_drift,1),'Holdout',0.3); 
% 
% % Extrai índices do teste 
% idx = cv.test;
% 
% % Usa índices para partição
% dadosTreino = steam_temperature_drift(~idx,:);
% dadosTeste = steam_temperature_drift(idx,:);
% 
