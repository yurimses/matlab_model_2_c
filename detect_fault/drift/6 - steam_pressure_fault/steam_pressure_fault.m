...
% LER E SALVAR
steam_pressure_drift = readtable('steam_pressure_drift5_1.csv');

%% DADOS

total_valores = sprintf("Total de valores = %d", height(steam_pressure_drift));
disp(total_valores);

%% DIVISÃO PARA TREINAMENTO E TESTE (SEQUENCIAL 70/30)

% Percentagem de treino
perc_treino = round(0.7*(height(steam_pressure_drift)));

% Atribuindo features (x) e saídas (y)
x = steam_pressure_drift(: , 2);
y = steam_pressure_drift(: , 3);
...
% cv = cvpartition(size(steam_pressure_drift,1),'Holdout',0.3); 
% 
% % Extrai índices do teste 
% idx = cv.test;
% 
% % Usa índices para partição
% dadosTreino = steam_pressure_drift(~idx,:);
% dadosTeste = steam_pressure_drift(idx,:);
% 
