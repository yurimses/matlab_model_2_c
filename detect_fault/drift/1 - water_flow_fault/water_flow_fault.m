...
% LER E SALVAR
water_flow_drift = readtable('water_flow_drift5_27.csv');

%% DADOS

total_valores = sprintf("Total de valores = %d", height(water_flow_drift));
disp(total_valores);

%% DIVISÃO PARA TREINAMENTO E TESTE (SEQUENCIAL 70/30)

% Percentagem de treino
perc_treino = round(0.7*(height(water_flow_drift)));

% Atribuindo features (x) e saídas (y)
x = water_flow_drift(: , 2);
y = water_flow_drift(: , 3);
...
% cv = cvpartition(size(water_flow_drift,1),'Holdout',0.3); 
% 
% % Extrai índices do teste 
% idx = cv.test;
% 
% % Usa índices para partição
% dadosTreino = water_flow_drift(~idx,:);
% dadosTeste = water_flow_drift(idx,:);
% 
...
%modelo = fitcsvm(x_treino, y_treino);
